...
// 获取省
var provlevel = document.getElementById('provlevel');
// 获取市
...
for (var i = 0; i < city.length; i++) {
    provlevel.options[provlevel.length] = new Option(city[i].name);
    // 循环第二步,把所有的市都循环进select
    provlevel.onchange = function () {
        citylevel.options.length = 0;
        citylevel.options[citylevel.length] = new Option("请选择城市");
        for (var j = 0; j < city[provlevel.selectedIndex - 1].city.length; j++) {
            citylevel.options[citylevel.length] = new Option(city[provlevel.selectedIndex - 1].city[j].name)
        }
    };
    citylevel.onchange = function () {
        arealevel.options.length = 0;
        arealevel.options[arealevel.length] = new Option("请选择区/县");
        for (var k = 0; k < city[provlevel.selectedIndex - 1].city[citylevel.selectedIndex - 1].area.length; k++) {
            arealevel.options[arealevel.length] = new Option(city[provlevel.selectedIndex - 1].city[citylevel.selectedIndex - 1].area[k]);
        }
    };
    arealevel.onchange = function () {
        // 获取固定地址的输入框
        var citysinfo = document.getElementById('citysinfo');
        // 省
        var index = provlevel.selectedIndex;
        var provinfo = provlevel.options[index].value;
        // 市
        var index1 = citylevel.selectedIndex;
        var cityinfo = citylevel.options[index1].value;
        // 区
        var index2 = arealevel.selectedIndex;
        var areainfo = arealevel.options[index2].value;
        // 将省市区写到固定的输入框不能修改
        citysinfo.value = (provinfo + cityinfo + areainfo);
    }
}